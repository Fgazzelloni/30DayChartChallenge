...
Here is a double check of the City names differences in `map_data("county")` and the `Lyme disease dataset` (`my_df2`). I am going to leave 96 city names behind.
```{r}
...
These are the set that will be used in the Map. 
...
Here is the part where the layout design is done. Use `{patchwork}` for setting the area to combine the title with the map with the `area()` function. Then chack the result with the `plot()` function, as shown below:
```{r}
library(patchwork)
# area(t, l, b = t, r = l)
design <- c(
